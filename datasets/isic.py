...
class ISIC2018DatasetFast(Dataset):
    def __init__(
        ...
        self.data_dir = (
            data_dir
            if data_dir
            else "/nvme-data/Medical/Segmentation_UNet/datasets/ISIC2018/np"
        )

        # input parameters
        self.one_hot = one_hot
        print("We are in ISIC2018DatasetFast at line 92")
        X = np.load(f"{self.data_dir}/X_tr_224x224.npy")
        Y = np.load(f"{self.data_dir}/Y_tr_224x224.npy")
        print("We are in ISIC2018DatasetFast at line 95")
        X = torch.tensor(X)
        Y = torch.tensor(Y)
...
class ISIC2018DatasetFast_Training(Dataset):
    def __init__(
        self, mode, data_dir=None, one_hot=True, img_transform=None, msk_transform=None
    ):
        # pre-set variables
        ...
        # input parameters
        self.one_hot = one_hot
        print("We are in ISIC2018DatasetFast at line 195")
        X = np.load(f"{self.data_dir}/X_tr_224x224.npy")
        Y = np.load(f"{self.data_dir}/Y_tr_224x224.npy")
        print("We are in ISIC2018DatasetFast at line 198")
        X = torch.tensor(X)
        Y = torch.tensor(Y)
...
class ISIC2018ValidationDataset(Dataset):
    def __init__(self, data_dir=None, img_transform=None, msk_transform=None):
        ...
        self.msks_dir = os.path.join(
            self.data_dir, "ISIC2018_Task1_Validation_GroundTruth"
        )
