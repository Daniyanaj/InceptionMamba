...
CONFIG_NAME = "segpc/segpc2021_mamba_Exp9_2.yaml"
...
from datasets.segpc_Augment import SegPC2021Dataset_train
...
tr_dataset = SegPC2021Dataset_train(
    mode="tr",
...
vl_dataloader = DataLoader(
    ...
    worker_init_fn=seed_worker,
    generator=g,
    **config["data_loader"]["validation"],
)

# prepare test dataloader
te_dataloader = DataLoader(
    te_dataset, worker_init_fn=seed_worker, generator=g, **config["data_loader"]["test"]
)
...
from models.mynet.mamba_Exp9_2 import MyNet as Net
...
with torch.no_grad():
    for batch in tqdm(te_dataloader):
        ...
        for idx in range(len(tbm)):
            img = np.moveaxis(txm[idx, :3], 0, -1) * 255.0
            nuc = np.uint8(txm[idx, -1] * 255.0)
            gt = np.uint8(tbm[idx] * 255.0)
